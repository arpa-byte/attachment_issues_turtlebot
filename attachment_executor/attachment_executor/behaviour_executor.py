...
class BehaviorExecutor(Node):
    ...
    def control_loop(self):
        ...
        if action == "FOLLOW_OWNER_HIGH_VEL":
            twist = self.go_to(self.owner_pose, max_linear=0.22, stop_dist=0.8)

        elif action == "FOLLOW_OWNER_MID_VEL":
            twist = self.go_to(self.owner_pose, max_linear=0.15, stop_dist=1.0)

        elif action == "FOLLOW_STRANGER_LOW_VEL":
            twist = self.go_to(self.stranger_pose, max_linear=0.10, stop_dist=1.2)
            ...
            if self.is_holding_toy:
                ...
                twist = self.go_to(self.toy_pose, max_linear=0.18, stop_dist=0.25)
            ...
            if not self.is_holding_toy:
                ...
                twist = self.go_to(self.owner_pose, max_linear=0.18, stop_dist=0.40)
                if self.dist_to(self.owner_pose) < 0.50:
                    self.is_holding_toy = False
                    self.send_feedback("SUCCESS")
